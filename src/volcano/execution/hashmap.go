...
func (hm *HashMap) Set(key common.Value, value interface{}) error {
	...
	for i := range list {
		if common.AreEqual(list[i].key, key) {
			list[i].value = value
			return nil
		}
	}
...
func (hm *HashMap) Get(key common.Value) (interface{}, bool, error) {
	hash, err := hashstructure.Hash(key, nil)
	if err != nil {
...
func (iter *Iterator) Next() (common.Value, interface{}, bool) {
