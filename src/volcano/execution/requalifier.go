...
func (node *Requalifier) Get(variables common.Variables) (RecordStream, error) {
...
func (stream *RequalifiedStream) Next() (*Record, error) {
	...
	fields := make([]common.VariableName, len(record.Fields()))
	values := make(map[common.VariableName]common.Value)
	for i := range oldFields {
		...
		qualifiedName := common.NewVariableName(fmt.Sprintf("%s.%s", stream.qualifier, name))
