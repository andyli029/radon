...
func ParseSelect(statement *sqlparser.Select, database string) (logical.Node, error) {
	var err error
	var root logical.Node
	...
	root, err = ParseTableExpression(statement.From[0], database, true)
	if err != nil {
	...
	aggregatesAs := make([]common.VariableName, len(statement.SelectExprs))
	aggregating := false
		...
		if _, ok := statement.SelectExprs[0].(*sqlparser.StarExpr); !ok {
			for i := range statement.SelectExprs {
				...
				if err == nil {
					...
					aggregatesAs[i] = common.NewVariableName(aliasedExpression.As.String())
					continue
				}
	...
	if aggregating {
		...
		fields := make([]common.VariableName, len(expressions))
		for i := range expressions {
			if !aggregateStars[i] {
			...
			for i := range expressions {
				if !aggregating {
					...
					if len(aggregatesAs[i]) > 0 {
						...
						if !aggregateStars[i] {
							nameExpressions[i] = logical.NewVariable(common.NewVariableName(fmt.Sprintf("%v_%v", expressions[i].Name(), aggregates[i])))
						} else {
							nameExpressions[i] = logical.NewVariable(common.NewVariableName(fmt.Sprintf("%v_%v", "*star*", aggregates[i])))
						}
					}
				}
			}
...
///*
func ParseTableExpression(expr sqlparser.TableExpr, database string, mustBeAliased bool) (logical.Node, error) {
	switch expr := expr.(type) {
	case *sqlparser.AliasedTableExpr:
		return ParseAliasedTableExpression(expr, database, mustBeAliased)
	case *sqlparser.JoinTableExpr:
		return ParseJoinTableExpression(expr, database)
	case *sqlparser.ParenTableExpr:
		return ParseTableExpression(expr.Exprs[0], database, mustBeAliased)
...
func ParseAliasedTableExpression(expr *sqlparser.AliasedTableExpr, database string, mustBeAliased bool) (logical.Node, error) {
	...
	case sqlparser.TableName:
		...
		if subExpr.Qualifier.String() != "" {
			database = subExpr.Qualifier.String()
		}
		return logical.NewDataSourceWithDatabase(subExpr.Name.String(), expr.As.String(), database), nil
...
func ParseJoinTableExpression(expr *sqlparser.JoinTableExpr, database string) (logical.Node, error) {
	leftTable, err := ParseTableExpression(expr.LeftExpr, database, true)
	if err != nil {
		return nil, errors.Wrap(err, "couldn't parse join left table expression")
	}
	rightTable, err := ParseTableExpression(expr.RightExpr, database, true)
	if err != nil {
	...
	if expr.On != nil {
		condition, err := ParseLogic(expr.On)
		if err != nil {
...
func ParseTableValuedFunction(expr *sqlparser.TableValuedFunction) (logical.Node, error) {
	...
	arguments := make(map[common.VariableName]logical.TableValuedFunctionArgumentValue)
	for i := range expr.Args {
		...
		arguments[common.NewVariableName(expr.Args[i].Name.String())] = parsed
	}
...
func ParseTableValuedFunctionArgument(expr sqlparser.TableValuedFunctionArgumentValue) (logical.TableValuedFunctionArgumentValue, error) {
	...
	case *sqlparser.FieldDescriptorTableValuedFunctionArgumentValue:
		...
		return logical.NewTableValuedFunctionArgumentValueDescriptor(common.NewVariableName(name)), nil
...
func ParseAliasedExpression(expr *sqlparser.AliasedExpr) (logical.NamedExpression, error) {
	...
	return logical.NewAliasedExpression(common.NewVariableName(expr.As.String()), subExpr), nil
}
...
func ParseExpression(expr sqlparser.Expr) (logical.Expression, error) {
	...
	case *sqlparser.ColName:
		...
		return logical.NewVariable(common.NewVariableName(name)), nil
