...
func (xc *XaCheck) WriteXaCommitErrLog(txn *Txn, state string) error {
	// if the xc is nil, it will be nothing to do
	if xc == nil {
		return nil
	}
...
// commitRetryBackends
// 1st. xa recover to check the xid is valid to some backends which need the 2nd command
...
func (xc *XaCheck) commitRetryBackends(query string, scatter *Scatter, xid string) (bool, error) {
	...
	for _, backend := range backends {
		result, _ := txn.ExecuteOnThisBackend(backend, xaRecoverQuery)
